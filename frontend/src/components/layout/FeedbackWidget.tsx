...
const FeedbackWidget = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createFeedback.mutate(
      { message: message.trim(), user_id: user.id },
      {
        onSuccess: () => {
          logAction('feedback', 'submit', '200')
          setSent(true)
          setMessage('')
          setTimeout(() => {
            setSent(false)
            ...
            <>
              ...
              {!user ? (
                <div className="p-4 text-center text-black/70 text-sm">
                  <p className="mb-3">Войдите в аккаунт, чтобы отправить обратную связь.</p>
                  <Link
                    to="/login"
                    className="font-unbounded font-semibold text-black underline hover:no-underline"
                  >
                    Войти
                  </Link>
                </div>
