...
import { useRegisterUser, useVerifyUser, apiUserToAuthUser } from '@/api/users'
...
const RegisterPage = () => {
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      const data = await registerMutation.mutateAsync({
        first_name: name,
        last_name: surname,
        father_name: patronymic || undefined,
        email,
        hash_password: password,
        role_id: role === 'seller' ? 2 : 1,
      })
      setCreatedUser(data)
      setStep('code')
    } catch (err) {
  ...
  const handleConfirmCode = async (e: React.FormEvent) => {
    ...
    if (!createdUser) {
      setCodeError('Сессия истекла, начните регистрацию заново')
      return
    }
    try {
      await verifyMutation.mutateAsync({ userId: createdUser.id, code: trimmed })
      setUser(apiUserToAuthUser(createdUser))
      createLog.mutate({ user_id: createdUser.id, system: 'auth', action: 'register', response: '200' })
      navigate('/')
    } catch {
      setCodeError('Неверный код или код истёк')
    }
  }
        ...
        <div className="rounded-3xl overflow-hidden shadow-2xl bg-white flex flex-col lg:flex-row min-h-[560px]">
          ...
          <div className="flex-1 p-8 lg:p-12 flex flex-col justify-center">
            {step === 'form' ? (
              ...
              >
                {registerMutation.isPending ? 'Отправка...' : 'Создать аккаунт'}
                <ArrowRight size={20} />
              </button>
            </form>
              ...
              <>
                ...
                <p className="text-black/60 text-sm mb-6">
                  Мы отправили 6-значный код на <strong>{createdUser?.email ?? email}</strong>. Введите его ниже.
                </p>
