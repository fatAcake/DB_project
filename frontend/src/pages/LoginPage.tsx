...
import { useUsers, apiUserToAuthUser } from '@/api/users'
...
const LoginPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (found) {
      setUser(apiUserToAuthUser(found))
      createLog.mutate({ user_id: found.id, system: 'auth', action: 'login', response: '200' })
      navigate('/')
    } else {
