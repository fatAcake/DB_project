...
from core.data import users_data, password_codes_data
...
# --- Коды смены пароля (хранятся в памяти с истечением 15 мин) ---

def _password_code_expires_at():
    return datetime.utcnow() + timedelta(minutes=15)


async def save_password_code(user_id: int, code: int) -> None:
    """Сохранить код смены пароля."""
    password_codes_data[user_id] = {"code": code, "expires_at": _password_code_expires_at()}


async def get_password_code(user_id: int) -> Optional[int]:
    """Получить код смены пароля (None если нет или истёк)."""
    data = password_codes_data.get(user_id)
    if not data:
        return None
    if datetime.utcnow() > data["expires_at"]:
        del password_codes_data[user_id]
        return None
    return data["code"]


async def delete_password_code(user_id: int) -> bool:
    """Удалить код смены пароля."""
    if user_id in password_codes_data:
        del password_codes_data[user_id]
        return True
    return False


async def verify_password_code(user_id: int, code: int) -> bool:
    """Проверить код смены пароля и при успехе удалить его."""
    stored = await get_password_code(user_id)
    if stored is None or stored != code:
        return False
    await delete_password_code(user_id)
    return True