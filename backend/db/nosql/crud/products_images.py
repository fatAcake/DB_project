...
class ProductsImagesCRUD:
    ...
    async def get(self, image_id: str) -> Optional[ProductImage]:
        """Получение изображения по ID (id проверяется перед запросом — защита от инъекций)."""
        oid = _safe_object_id(image_id)
        if oid is None:
            return None
    ...
    async def delete(self, image_id: str) -> bool:
        """Удаление изображения (id проверяется перед запросом — защита от инъекций)."""
        oid = _safe_object_id(image_id)
        if oid is None:
            return False
